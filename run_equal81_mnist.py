...
X_val = np.reshape(X_val,(X_val.shape[0],28,28,1))[::11]
y_train = mnist.train.labels
y_val = mnist.test.labels[::11]
...
#    Y_pred = np.concatenate([Y_pred0,Y_pred1],0)
    print Y_pred.shape
    print (Y_pred[0])
    Y_pred = np.log(Y_pred/(1-Y_pred))-np.log(0.333) #linearize probs
    print len(Y_pred)
    print (preds_1)
    #φ = (X T W X) −1 X T W y
    ...
    for num_of_chosen in range(1,counter):
        if num_of_chosen < counter-1:
            ...
            #sample_index += list(itertools.combinations(range(max_val), num_of_chosen))
             ...
             sample_index += list(itertools.combinations(range(max_val), max_val
-num_of_chosen))[::11]
